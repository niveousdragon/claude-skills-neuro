...
7. **Package-generated sections are rebuilt.** `nomencl` collects
   `\nomenclature{term}{definition}` entries and prints them where
   `\printnomenclature` sits. Pandoc knows neither command, so the whole list of
   abbreviations would vanish without a word of warning. The entries are read
   from the source and emitted as a definition list, under the heading from
...
| A whole section is missing (abbreviations, glossary, index) | It was generated by a package, not written out. `nomencl` is handled; for `glossaries`/`acronym`/`makeindex` you must still check by hand. |
| The `.docx` has far fewer pages than the PDF | Usually typography, not loss: a GOST thesis is 14 pt with 1.5 spacing, Word's default is 12 pt single — roughly 1.3× more text per page. Compare *word counts*, not page counts, before worrying. |
